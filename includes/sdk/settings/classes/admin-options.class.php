...
if ( ! class_exists( 'PBSettings_Options' ) ) {
	class PBSettings_Options extends PBSettings_Abstract {
		...
		public function set_options( $ajax = false ) {
			...
			$options   = ( ! empty( $response[ $this->unique ] ) ) ? array_map( 'sanitize_text_field', $response[ $this->unique ] ) : array();
			...
			if ( $has_nav ) {
				...
				foreach ( $this->pre_tabs as $tab ) {
					...
					if ( ! empty( $tab['subs'] ) ) {
						...
						echo '<a href="#tab=' . esc_attr( $tab_id ) . '" data-tab-id="' . esc_attr( $tab_id ) . '" class="pbsettings-arrow">' . esc_html($tab_icon . $tab['title'] . $tab_error) . '</a>';
						...
						foreach ( $tab['subs'] as $sub ) {
							...
							echo '<li><a href="#tab=' . esc_attr( $sub_id ) . '" data-tab-id="' . esc_attr( $sub_id ) . '">' . esc_html($sub_icon . $sub['title'] . $sub_error) . '</a></li>';
